...
class LeaderboardUI {
    ...
    updateLeaderboard() {
        ...
        if (leaderboard.length === 0) {
            tbody.innerHTML = '<tr><td colspan="5" class="no-data">No records yet</td></tr>';
            return;
        }
        ...
        personalBestInfo.innerHTML = `
            <div class="best-stats">
                <div class="stat-item">
                    ...
                    <span class="stat-value">${bestScore} pts</span>
                </div>
                <div class="stat-item">
...
class StatisticsDashboard {
    ...
    updateRecentTrend(recentGames) {
        ...
        if (recentGames.length === 0) {
            container.innerHTML = '<p class="no-data">No game records yet</p>';
            return;
        }
        ...
        recentGames.forEach((game, index) => {
            ...
            bar.innerHTML = `
                <div class="bar-value" style="height: ${barHeight}%">
                    <span class="tooltip">${game.score} pts (${accuracy}%)</span>
                </div>
    ...
    updateAchievements(achievementIds) {
        ...
        const allAchievements = {
            'first_game': { name: 'First Challenge', description: 'Completed your first quiz game', icon: '🎯' },
            'perfect_score': { name: 'Perfect Score', description: 'Answered every question correctly in a game of 10+ questions', icon: '⭐' },
            'streak_master': { name: 'Streak Master', description: '10 correct answers in a row', icon: '🔥' },
            'quiz_veteran': { name: 'Quiz Veteran', description: 'Played 50 games', icon: '🎖️' },
            'knowledge_seeker': { name: 'Knowledge Seeker', description: 'Played every category', icon: '📚' }
        };

        container.innerHTML = '';

        if (achievementIds.length === 0) {
            container.innerHTML = '<p class="no-data">No achievements earned yet</p>';
            return;
        }
