    ...
    constructor(openRouterClient, config = {}) {
        ...
        this.modes = {
            brief: {
                ...
                description: 'Just the essentials, briefly'
            },
            standard: {
                maxTokens: 800,
                temperature: 0.3,
                summaryRatio: 0.2, // 20% of original length
                description: 'Key content, balanced'
            },
            detailed: {
                maxTokens: 1500,
                temperature: 0.3,
                summaryRatio: 0.3, // 30% of original length
                description: 'Includes detailed content'
            }
        };
...
**Target Length**: Approximately ${Math.round(modeConfig.summaryRatio * 100)}% of the original

**Summarization Principles**:
1. Extract only core information and key arguments
2. Maintain the logical flow and structure of the original
3. Include only objective and accurate information
4. Remove unnecessary details and repetitive content
5. Write in clear, easy-to-understand English
...
**Important**: Follow the format above exactly, and respond in English only.`;
        } else {
            return `You are a professional document summarization AI. Follow these guidelines strictly:
...
**Important**: Follow the format exactly and maintain consistency.`;
    ...
    async summarizeChunk(text, mode = 'standard', options = {}) {
        ...
        const messages = [
            ...
            {
                role: 'user',
                content: `Please summarize the following text in ${mode} mode. Respond in English:\n\n${text}`
            }
        ];
    ...
    async combineChunkSummaries(summaries, mode = 'standard', originalLength) {
        ...
        const systemPrompt = `You are an expert at combining multiple partial summaries into one coherent final summary. Respond in English.

**Goals**:
- Merge the partial summaries into one complete summary
- Remove duplicated content and build a logical flow
- Keep the length and level of detail appropriate for ${mode} mode
...
\`\`\``;
        ...
        const messages = [
            ...
            {
                role: 'user',
                content: `The following are summaries of separate sections of a long document. Please merge them into one complete final summary. Respond in English:\n\n${combinedText}`
            }
        ];
    ...
    async summarize(text, mode = 'standard', options = {}) {
        ...
        try {
            ...
            throw new Error(`An error occurred while generating the summary: ${error.message}`);
        }
    }
