...
const categories = {
    work: { name: 'Work', color: '#f56565', icon: '💼' },
    personal: { name: 'Personal', color: '#48bb78', icon: '🏠' },
    study: { name: 'Study', color: '#4299e1', icon: '📚' }
};
...
// ==================== Motivational Quotes ====================
const quotes = [
    "Completing small tasks is the beginning of great things.",
    "Today's small efforts create tomorrow's big results.",
    "Execution matters more than a perfect plan.",
    "One step at a time will get you to your goal.",
    "Those who never give up succeed in the end.",
    "Don't put off until tomorrow what you can do today.",
    "Success is the sum of small daily efforts.",
    "Well begun is half done. Start now!",
...
function deleteTask(id) {
    ...
    if (taskIndex !== -1) {
        ...
        saveTasks();
        updateUI();
        showNotification('Task deleted. (Ctrl+Z to undo)', 'info');
    }
}
...
function clearCompletedTasks() {
    ...
    if (completedTasks.length === 0) {
        showNotification('There are no completed tasks to delete.', 'info');
        return;
    }
    ...
    if (confirm(`Delete ${completedTasks.length} completed task(s)?`)) {
        ...
        saveTasks();
        updateUI();
        showNotification(`${completedTasks.length} task(s) deleted.`, 'success');
    }
}
...
function suggestCategory(text) {
    ...
    if (detectedCategory && detectedCategory !== elements.categorySelect.value) {
        ...
        suggestionText.textContent = `Classify this under the "${categories[detectedCategory].name}" category?`;
...
function applySuggestedCategory() {
    ...
    if (suggestedCategory) {
        ...
        elements.categorySuggestion.style.display = 'none';
        showNotification(`Category set to "${categories[suggestedCategory].name}".`, 'success');
    }
}
...
function renderTasks() {
    ...
    sortedTasks.forEach(task => {
        ...
        date.textContent = isToday ? 'Today' : formatDate(taskDate);
    ...
    elements.todoCount.textContent = `${sortedTasks.length} item(s)`;
}
...
function importTasks(event) {
    ...
    reader.onload = (e) => {
        try {
            ...
            showNotification('Unable to read the file.', 'error');
...
function saveKeywords() {
    ...
    saveCustomKeywords();
    closeKeywordModal();
    showNotification('Keywords saved!', 'success');
}
...
function showMotivationalMessage() {
    const messages = [
        ...
        "Awesome! Knocking them out one by one! ⭐",
        "Amazing! You're getting closer to your goal! 🚀"
    ];
